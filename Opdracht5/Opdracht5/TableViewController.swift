...
class TableViewController: UITableViewController {
    ...
    let _api : AlamofireApiService = Extensions.appDelegate._api
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        //let _api = AlamofireApiService.instance
        
        _api.GetArticles({ (results) -> () in
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        cell.Description.text = _articles[indexPath.row].Summary
    ...
    override func tableView(tableView: UITableView, willDisplayCell cell: UITableViewCell, forRowAtIndexPath indexPath: NSIndexPath) {
        if(indexPath.row >= self._articles.count - 5){
            ...
            _api.GetNextArticles(self.nextId, callbackSuccess: { (results) -> () in
                let _result: ResultModel = results
                self._articles.appendContentsOf(_result.Results)
                self.nextId = _result.NextId
                self.tableView.reloadData()
